...
class SimpleNeuralNetwork:
    def __init__(self, input_size=6, hidden_size=8, output_size=4):
        """
        Simple neural network for genetic algorithm with 2 hidden layers
        input_size: 6 (speed, angle, 3 sensors)
        hidden_size: 8 nodes per hidden layer
        ...
        self.w2 = np.random.randn(hidden_size, hidden_size) * 0.5  # New hidden layer
        self.b2 = np.random.randn(hidden_size) * 0.5               # New bias
        self.w3 = np.random.randn(hidden_size, output_size) * 0.5  # Changed from w2
        self.b3 = np.random.randn(output_size) * 0.5              # Changed from b2
    
    def forward(self, x):
        """Forward pass through network with 2 hidden layers"""
        # First hidden layer
        h1 = np.tanh(np.dot(x, self.w1) + self.b1)
        # Second hidden layer
        h2 = np.tanh(np.dot(h1, self.w2) + self.b2)
        # Output layer
        output = 1 / (1 + np.exp(-(np.dot(h2, self.w3) + self.b3)))
        return output
    ...
    def set_weights(self, weights):
        ...
        # w2 (new hidden layer)
        w2_size = self.hidden_size * self.hidden_size
        self.w2 = weights[idx:idx+w2_size].reshape(self.hidden_size, self.hidden_size)
        idx += w2_size
        
        # b2 (new hidden layer)
        self.b2 = weights[idx:idx+self.hidden_size]
        idx += self.hidden_size
        
        # w3 (output layer, was previously w2)
        w3_size = self.hidden_size * self.output_size
        self.w3 = weights[idx:idx+w3_size].reshape(self.hidden_size, self.output_size)
        idx += w3_size
        
        # b3 (output layer, was previously b2)
